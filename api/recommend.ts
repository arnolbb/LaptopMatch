...
const RATE_LIMIT_MAX = 10;           // max 10 requests per window (relaxed for serverless cold starts)
...
function validateInput(body: unknown): { ok: true; data: ValidatedInput } | { ok: false; error: string } {
  ...
  if (!Array.isArray(b.needs) || b.needs.length === 0 || b.needs.length > 8) {
    return { ok: false, error: "Pilih 1?8 kebutuhan utama." };
  }
...
export default async function handler(req: any, res: any) {
  ...
  const limit = checkRateLimit(String(ip));
  ...
  try {
    ...
    if (!ai) {
      return res.status(500).json({ error: "GEMINI_API_KEY belum dikonfigurasi di environment Vercel." });
    }
...
Dan tambahkan satu paragraf singkat "Saran Pembelian" di akhir.`;
    ...
    console.error("Error generating recommendations in serverless handler:", message);
    ...
    return res.status(500).json({ error: "Gagal menghasilkan rekomendasi. Silakan coba lagi." });
  }
}
